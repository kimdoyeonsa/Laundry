...
public class LaundryDAO {
	...
	try {
		...
		pstmt.execute();
		sql = new StringBuilder();
		sql.append("create table if not exists Laundry."+table+"(id int(255) not null AUTO_INCREMENT,dong varchar(10),hosu varchar(10) not null default 0,name varchar(20),phone varchar(15),work varchar(100),amount varchar(10),pay enum('선불','후불'),output enum('입고','출고'),date datetime default now(),primary key(id));");
		pstmt=conn.prepareStatement(sql.toString());
		pstmt.execute();
	}catch(Exception e) {
		e.printStackTrace();
